...
from app.core.prompts import TASK_GENERATION_SYSTEM, get_task_generation_prompt
...
class TaskGeneratorAgent:
    ...
    async def generate_task(
        ...
        progress_str = (
            json.dumps(developer_progress, indent=2) if developer_progress else "{}"
        )
        ...
        result = self.gemini.generate_json(
            prompt, TASK_GENERATION_SYSTEM, use_flash=True
        )
    ...
    def generate_trace_flow_task(
        self, entry_point: str, target_output: str
    ) -> LearningTask:
        ...
        return LearningTask(
            ...
            hints=[
                TaskHint(
                    1,
                    "Use your IDE's 'Go to Definition' feature to follow function calls",
                    10,
                ),
    ...
    def generate_safe_modification_task(
        self, target_file: str, modification_type: str = "enhancement"
    ) -> LearningTask:
        ...
        return LearningTask(
            ...
            hints=[
                TaskHint(
                    1,
                    "Small changes are fine! Even improving one error message counts.",
                    10,
                ),
                TaskHint(
                    2, "Before changing, make sure you understand the existing code", 20
                ),
                TaskHint(
                    3, "If tests fail, use the error message to understand why", 30
                ),
            ],
    ...
    async def generate_tasks(
        self, code_graph: CodeGraph, total_tasks: int = 5
    ) -> List[Dict[str, Any]]:
        """
                ...
                modules.append(
                    {
                        ...
                        "name": node.name,
                        "difficulty": difficulty,
                        "content": node.metadata.get(
                            "embedding", ""
                        ),  # Using embedding as proxy for now, but usually we need content
            ...
            TaskType.SCAVENGER_HUNT: [
                {
                    ...
                    "objective": "Locate where the application starts",
                },
                {"title": "Map the API", "objective": "Document all API endpoints"},
                {
                    "title": "Discover the Data Models",
                    "objective": "Find and list all data models",
                },
            ],
            TaskType.TRACE_FLOW: [
                {
                    "title": "Request Flow",
                    "objective": "Trace an HTTP request through the system",
                },
                {
                    "title": "Data Processing",
                    "objective": "Follow data transformation pipeline",
                },
            ...
            TaskType.DOCUMENTATION: [
                {
                    ...
                    "objective": "Add comprehensive docstrings",
                },
                {
                    "title": "Create a README Section",
                    "objective": "Improve project documentation",
                },
                {
                    "title": "Comment Complex Logic",
                    "objective": "Explain tricky code sections",
                },
            ],
        }

    def _parse_task_result(
        self, result: Dict[str, Any], module_info: Dict[str, Any]
    ) -> LearningTask:
            ...
            "hints": [
                {
                    ...
                    "content": h.content,
                    "reveal_after": h.reveal_after_minutes,
                }
                for h in task.hints
            ],
