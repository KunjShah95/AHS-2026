...
class InteractiveTutorAgent:
    ...
    async def explain_code(
        self, file_path: str, code_content: str, explanation_style: str = "eli_junior"
    ) -> str:
        """
        ...
        style_instruction = style_prompts.get(
            explanation_style, style_prompts["eli_junior"]
        )
...
"""
        ...
        response = self.gemini.generate_text(
            prompt, INTERACTIVE_TUTOR_SYSTEM, use_flash=True, temperature=0.2
        )
...
"""
    ...
    async def get_concept_explanation(
        self, concept: str, codebase_examples: List[str]
    ) -> Dict[str, Any]:
        """
...
"""
    ...
    def _format_codebase_context(self, context: Dict[str, Any]) -> str:
        ...
        if "modules" in context:
            ...
            for module in context["modules"][:15]:
                lines.append(
                    f"- {module.get('path', 'unknown')}: {module.get('responsibility', 'Unknown')}"
                )
    ...
    def _format_developer_profile(self, progress: Optional[Dict[str, Any]]) -> str:
        ...
        return (
            "\n".join(profile_lines)
            if profile_lines
            else "Junior developer, recently started onboarding."
        )
    ...
    def _enhance_response(
        self, response_text: str, codebase_context: Dict[str, Any], question: str
    ) -> TutorResponse:
        ...
        for module in modules:
            ...
            if path and (
                path in response_text or os.path.basename(path) in response_text
            ):
                references.append(
                    {
                        "path": path,
    ...
    def _generate_follow_ups(self, question: str, context: Dict[str, Any]) -> List[str]:
        ...
        if "why" in question_lower:
            follow_ups.append(
                "Want me to explain alternative approaches and why this one was chosen?"
            )

        if "error" in question_lower or "bug" in question_lower:
            follow_ups.append(
                "Should I help you understand common pitfalls in this area?"
            )
    ...
    def get_conversation_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_interaction": messages[-1].timestamp.isoformat()
            if messages
            else None,
