...
class CodebaseArchitectAgent:
    ...
    async def analyze_repository(
        self, root_path: str, file_tree: List[Dict], max_modules_to_analyze: int = 15
        ...
        priority_modules = self._select_priority_modules(
            file_tree, max_modules_to_analyze
        )
        ...
        return ArchitectureAnalysis(
            architecture_type=self._parse_architecture_type(
                arch_result.get("architecture_type", "unknown")
            ),
    ...
    def _select_priority_modules(self, file_tree: List[Dict], limit: int) -> List[Dict]:
        ...
        for file_info in file_tree:
            ...
            if any(
                core in path.lower()
                for core in ["app", "src", "core", "api", "services"]
            ):
                score += 5
            ...
            if any(
                skip in path.lower()
                for skip in ["test", "spec", "node_modules", "__pycache__", ".git"]
            ):
                score -= 20
    ...
    async def _analyze_modules(
        self, root_path: str, modules: List[Dict]
    ) -> List[ModuleAnalysis]:
        ...
        for module in modules:
            try:
                full_path = module.get("full_path") or os.path.join(
                    root_path, module["path"]
                )
                ...
                imports = self._extract_imports(
                    content, module.get("language", "unknown")
                )
                ...
                analysis = ModuleAnalysis(
                    path=module["path"],
                    primary_responsibility=result.get(
                    ...
                    eli_junior_summary=result.get(
                        "eli_junior_summary", "No summary available"
                    ),
