...
class ProgressCoachAgent:
    ...
    def initialize_progress(
        self, user_id: str, total_phases: int = 5
    ) -> DeveloperProgress:
    ...
    def record_task_completion(
            ...
            for concept in concepts_learned:
                if concept not in progress.concept_mastery:
                    progress.concept_mastery[concept] = ConceptMastery(
                        concept_name=concept, exposure_count=0, practice_count=0
                    )
        ...
        return {
            ...
            "estimated_days_remaining": (
                progress.estimated_completion_date - datetime.now()
            ).days
    ...
    async def get_personalized_feedback(self, user_id: str) -> ProgressFeedback:
        ...
        result = self.gemini.generate_json(
            prompt, PROGRESS_COACH_SYSTEM, use_flash=True
        )

        return ProgressFeedback(
            greeting=result.get("greeting", f"Hey there! 👋"),
            recent_win=result.get(
                "recent_win", "You've started your onboarding journey!"
            ),
            progress_summary=result.get(
                "progress_summary",
                {
                    ...
                    "strongest_area": progress.strengths[0]
                    if progress.strengths
                    else "Exploration",
                    "growth_area": progress.growth_areas[0]
                    if progress.growth_areas
                    else "Still learning",
                },
            ),
            ...
            improvement_tip=result.get(
                "improvement_tip",
                "Take notes as you explore - they'll be valuable later!",
            ),
    ...
    def get_team_analytics(self, user_ids: List[str]) -> Dict[str, Any]:
        ...
        avg_tasks = sum(p["tasks_completed"] for p in team_progress) / len(
            team_progress
        )
        avg_hours = sum(p["time_spent_hours"] for p in team_progress) / len(
            team_progress
        )
    ...
    def get_leaderboard(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        sorted_users = sorted(
            self.progress_store.values(), key=lambda p: p.total_xp, reverse=True
        )[:limit]
    ...
    def _check_badge_rewards(self, progress: DeveloperProgress) -> List[str]:
        ...
        if progress.completed_tasks:
            ...
            if (
                last_task.time_spent_minutes < 15
                and "Speed Demon 💨" not in progress.badges
            ):
                new_badges.append("Speed Demon 💨")
            if (
                last_task.hints_used == 0
                and "No Hints Needed 🧠" not in progress.badges
            ):
    ...
    def _calculate_velocity(self, progress: DeveloperProgress) -> LearningVelocity:
        ...
        if len(recent) >= 2:
            ...
            for i in range(1, len(recent)):
                gap = (
                    recent[i].completed_at - recent[i - 1].completed_at
                ).total_seconds() / 3600
                gaps.append(gap)
    ...
    def _generate_team_insights(self, team_progress: List[Dict]) -> List[str]:
        ...
        if stalled:
            insights.append(
                f"⚠️ {len(stalled)} developer(s) may need additional support"
            )
        ...
        if accelerating:
            insights.append(
                f"🌟 {len(accelerating)} developer(s) are ahead of schedule"
            )
        ...
        if avg_hours < 5:
            insights.append(
                "💡 Team average onboarding time is low - consider scheduling dedicated time"
            )
        elif avg_hours > 30:
    ...
    def to_dict(self, progress: DeveloperProgress) -> Dict[str, Any]:
        ...
        return {
            ...
            "estimated_completion": progress.estimated_completion_date.isoformat()
            if progress.estimated_completion_date
            else None,
